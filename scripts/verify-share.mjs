...
const paths = {
  career:'tests/career/app.js', quick:'tests/quick/app.js', insight:'tests/insight/app.js', compatibility:'compatibility/app.js', dreams:'astro/dreams/app.js', antiIq:'tests/anti-iq/app.js'
};
...
for (const name of ['career','quick','insight','antiIq']) {
...
for (const link of ['comics/','tests/anti-iq/','tests/career/index.html','test=strengths','test=trajectory','test=numerology','compatibility/','syutsai/','horoscope/','astro/dreams/','modules/dice-fate.html','test=team','test=decisions','test=environment','test=battery','test=tabs','test=animal','test=lifeAnimal','test=gamer']) {
  registry.some(item => item.path.includes(link)) || fail(`registry missing ${link}`);
}
/app\.js\?v=\d+/.test(home) || fail('home does not load the dynamic hub');
...
for (const asset of ['app.js','styles.css','tests-registry.json','hub-locales.json','comics/index.html','comics/bubbles.css','comics/night-noir/frame-06.webp','comics/eclipse-protocol/frame-06.webp','comics/anatomy-of-feelings/frame-06.webp','tests/anti-iq/index.html','tests/anti-iq/app.js','astro/dreams/index.html','astro/dreams/app.js','astro/dreams/data/objects.json','astro/dreams/locales/kk.json']) sw.includes(asset) || fail(`PWA missing ${asset}`);
...
console.log('OK: share links, dynamic registry hub, privacy-safe GA4 events, SEO discovery, and versioned PWA cache');
