...
async function ptUpdateApp(){
  const copy=ptCopy();
  try{
    const registration=await navigator.serviceWorker?.register(`${PT_ROOT}service-worker.js?v=1140`,{scope:PT_ROOT,updateViaCache:'none'});
...
function ptEnsurePwa(){
  ...
  navigator.serviceWorker?.register(`${PT_ROOT}service-worker.js?v=1140`,{scope:PT_ROOT,updateViaCache:'none'}).catch(()=>{});
}
