...
const PT_SUPPORT_COPY={
  ru:{title:'🤖 ИИ съел токены — PortHub просит добавки',text:'Новые тесты, игры и исправления требуют времени, кофе и платных сервисов. Если PortHub вас развлёк, поддержите проект через Boosty или Ko-fi — любая сумма приблизит следующее обновление. Не готовы донатить? Просто поделитесь приложением: это тоже большая помощь. Возможно, PortHub зайдёт вашему другу, и кофе разработчику прилетит уже от него 🙂',boosty:'Поддержать через Boosty',kofi:'Поддержать через Ko-fi'},
  kk:{title:'🤖 ЖИ токендерді жеп қойды — PortHub-қа қосымша қуат керек',text:'Жаңа тесттер, ойындар мен түзетулерге уақыт, кофе және ақылы сервистер қажет. PortHub ұнаса, жобаны Boosty немесе Ko-fi арқылы қолдаңыз — кез келген сома келесі жаңартуды жақындатады. Донат жасауға дайын емессіз бе? Қолданбамен бөлісудің өзі үлкен көмек.',boosty:'Boosty арқылы қолдау',kofi:'Ko-fi арқылы қолдау'},
  en:{title:'🤖 AI ate the tokens — PortHub needs a refill',text:'New tests, games and fixes take time, coffee and paid tools. If PortHub entertained you, support the project through Boosty or Ko-fi — any amount helps the next update arrive sooner. Not ready to donate? Sharing the app helps too. A friend may love PortHub and buy the developer that coffee 🙂',boosty:'Support via Boosty',kofi:'Support via Ko-fi'},
  fr:{title:'🤖 L’IA a mangé les jetons — PortHub en redemande',text:'Les nouveaux tests, jeux et correctifs demandent du temps, du café et des services payants. Si PortHub vous a diverti, soutenez le projet via Boosty ou Ko-fi : chaque don rapproche la prochaine mise à jour. Pas prêt à donner ? Partager l’application nous aide aussi.',boosty:'Soutenir via Boosty',kofi:'Soutenir via Ko-fi'}
};
...
function ptInstallStyles(){
  ...
  style.textContent=`
    ...
    .pt-support-callout{margin:18px 0;padding:18px;border:1px solid #7c5cb7;border-radius:18px;background:linear-gradient(145deg,#34244d,#241b36);box-shadow:0 14px 32px #0b071533}.pt-support-callout h2{margin:0 0 8px;color:#fff}.pt-support-callout p{margin:0;color:#d8d0e4}.pt-support-actions,.pt-footer-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px;margin-top:14px}.pt-footer-actions{margin-top:22px}.pt-footer-button{display:flex;align-items:center;justify-content:center;min-height:50px;padding:11px 14px;border:1px solid #514964;border-radius:15px;background:#2c2643;color:#fff!important;font-size:.92rem;font-weight:800;text-align:center;text-decoration:none;cursor:pointer}.pt-footer-button.primary{border-color:#f2cb75;background:#f2cb75;color:#20182d!important}.pt-support-callout .pt-footer-button{border-color:#f2cb75;background:#f2cb75;color:#20182d!important}.pt-support-callout .pt-support-share{grid-column:1/-1;border-color:#67577f;background:#2c2643;color:#fff!important}.pt-footer-button:hover,.pt-footer-button:focus-visible{outline:3px solid #9c8aff55;outline-offset:2px}.pt-footer-button[data-pt-update]{grid-column:1/-1}
...
async function ptUpdateApp(){
  const copy=ptCopy();
  try{
    const registration=await navigator.serviceWorker?.register(`${PT_ROOT}service-worker.js?v=1178`,{scope:PT_ROOT,updateViaCache:'none'});
...
function ptEnsurePwa(){
  ...
  navigator.serviceWorker?.register(`${PT_ROOT}service-worker.js?v=11711`,{scope:PT_ROOT,updateViaCache:'none'}).catch(()=>{});
}
